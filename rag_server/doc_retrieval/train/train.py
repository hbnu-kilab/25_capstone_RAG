...
def argument_parser():
    ...
    parser.add_argument('--q_encoder_path', type=str, default='path/to/query_encoder')
    parser.add_argument('--c_encoder_path', type=str, default='path/to/context_encoder')
...
def main(args):
    ...
    # ✅ Query Encoder & Context Encoder 각각 불러오기
    q_encoder = AutoModel.from_pretrained(args.q_encoder_path)
    c_encoder = AutoModel.from_pretrained(args.c_encoder_path)

    config = AutoConfig.from_pretrained(args.q_encoder_path)
    tokenizer = AutoTokenizer.from_pretrained(args.q_encoder_path, config=config)

    if args.device == 'cuda':
            biencoder = BiEncoder(args, q_encoder=q_encoder, c_encoder=c_encoder).to(args.device)
            optimizer = get_adamw_optimizer(biencoder.q_encoder, biencoder.c_encoder, args)
            LOGGER.info("Using single GPU (no DataParallel)")
    else:
        biencoder = BiEncoder(args, q_encoder=q_encoder, c_encoder=c_encoder)
        optimizer = get_adamw_optimizer(biencoder.q_encoder, biencoder.c_encoder, args)
    ...
    for epoch_i in range(args.epochs):
        ...
        if epoch_i % args.eval_epoch == 0:
            ...
            # q_output_path = os.path.join(args.q_output_path, f"epoch{epoch_i+1}_top1_{top1_acc:.4f}")
            # c_output_path = os.path.join(args.c_output_path, f"epoch{epoch_i+1}_top1_{top1_acc:.4f}")
            q_output_path = args.q_output_path
            c_output_path = args.c_output_path

            os.makedirs(q_output_path, exist_ok=True)
            os.makedirs(c_output_path, exist_ok=True)

            model_to_save.save_model(q_output_path, c_output_path)

            LOGGER.info(f'>>> Saved Model (Question Encoder) at {q_output_path}')
            LOGGER.info(f'>>> Saved Model (Context Encoder) at {c_output_path}')
