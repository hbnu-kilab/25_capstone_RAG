...
class VectorDatabase(object):
    ...
    def _load_context_from_json(self, json_path):
        print('>>> Loading context data from JSON.')
        ...
        return idx_list, text_list, title_list
    ...
    def build_embedding(self,
        ...
        idx_lst, txt_lst, title_lst = self._load_context_from_json(wiki_path)
        ...
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        with open(os.path.join(save_path, 'faiss_pickle.pkl'), 'wb') as file:
            pickle.dump({
                'text_index': idx_lst,
                'faiss_index': faiss_index,
            }, file)
        ...
        if save_context:
            print(">>> Saving context pickle. It contains 'title' and 'text'.")
            with open(os.path.join(save_path, 'context_pickle.pkl'), 'wb') as file:
                pickle.dump({
                    'title': title_lst,
                    'text': txt_lst,
                }, file)
